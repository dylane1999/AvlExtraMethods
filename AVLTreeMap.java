...
public class AVLTreeMap implements Map {
   ...
   // private int size;
    ...
    public boolean isDestructiveSwap(){
        return false;
    }
    ...
    private int sum(Node node, int runningCount ){
        if (node == null) return 0;
        else {
            int value = Integer.parseInt(node.key.trim());
            runningCount = value + runningCount;
            if (node.right ==null & node.left ==null) {
                return runningCount;
            }
            int right = sum(node.right, runningCount);
            int left = sum(node.left, right);
            return left;
        }
...
//11
    ...
    public static void main(String[] args) {
        ...
        System.out.println(map.sum(map.root));

        map.inOrder();
        map.swap();
        System.out.println(" ");
        map.inOrder();
        map.isDestructiveSwap();
